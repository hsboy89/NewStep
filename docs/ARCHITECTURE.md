...
### 1. 로컬 스토리지 캐싱
- **기사 목록 캐싱**: 기사 목록과 단어장을 로컬 스토리지에 저장하여 재방문 시 빠른 로딩
- **캐시 유효 시간**: 1시간 (rss2json API 갱신 주기와 동일)
- **스마트 로딩**: 캐시가 유효하면 API 호출 생략, 만료된 경우에만 새로고침
- **API 호출 감소**: 약 95% 감소 (432회/일 → 24회/일)

### 2. 조건부 렌더링
- WordTooltip은 단어 클릭 시에만 렌더링
- Dither 컴포넌트 제거로 불필요한 Three.js 의존성 제거

### 3. 자동 업데이트 최적화
- **업데이트 간격**: 1시간마다 체크 (기존 10분 → 1시간)
- **캐시 확인**: 자동 업데이트 시에도 캐시 유효성 확인 후 API 호출 결정
- **수동 새로고침**: 사용자가 새로고침 버튼 클릭 시에만 강제로 API 호출
